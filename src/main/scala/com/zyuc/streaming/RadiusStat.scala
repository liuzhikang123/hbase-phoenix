...
/**
  * Created by zhoucw on 18-12-28 下午4:20
  */
object RadiusStat {
  def main(args: Array[String]): Unit = {
    ...
    def statPgw() = {
      ...
      insert2Tidb(tidbpgw, pgwradiusout)
      CommonUtils.updateBreakTable(tidbpgw, stattime+"00")
    }

    def stat3g() = {

      val inputHaPath = haradiuspath + "/" + "d=" + d + "/h=" + h + "/m5=" + m5

      spark.read.format("orc").load(inputHaPath).createOrReplaceTempView(haTable)
      ...
      insert2Tidb(tidbha, haradiusout)
      //更新 断点表
    ...
    def statNb() = {
      ...
      insert2Tidb(tidbnb, nbradiusout)
    ...
    if(stattype == "0"){
      ...
      spark.read.format("orc").load(inputPgwPath).createOrReplaceTempView(pgwTable)
      set4gBsTable()
      if(stattype == "2"){
        statPgw()
      }else{
