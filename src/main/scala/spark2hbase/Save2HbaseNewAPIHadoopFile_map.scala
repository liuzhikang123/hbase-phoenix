...
count 'spark2hbase_test'
  */
object Save2HbaseNewAPIHadoopFile_map {
  def main(args: Array[String]): Unit = {
    ...
    val rdd = sc.textFile(input + "/"+dataTime+"/").map(x => parse(x))
    ...
    val result1 = rdd.map(x => {
      ...
      //val cf: Array[Byte] = "cf1".getBytes
      val cf: Array[Byte] = Bytes.toBytes("cf1")
      val rowkey = Bytes.toBytes(x("key"))

      x.-("key").keys.foreach(i=>{
        //cn = i.getBytes()
        cn = Bytes.toBytes(i)
        v = Bytes.toBytes(x(i))
  ...
  def parse(line: String): ListMap[String,String] = {
    try {
      ...
      val key = mdnReverse + "_" + StartTime + "_" + EndTime

      var A:Map[String,String] = Map()

      A += ("key" -> key)
      for(i<-0 to 63){
        A += (columnsName(i) -> fields(i))
      }

      ListMap(A.toSeq.sortBy( _._1 ) :_ *)
