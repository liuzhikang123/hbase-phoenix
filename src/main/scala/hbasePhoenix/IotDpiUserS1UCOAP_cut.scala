...
//      "Proxy_Scheme",
  ...
  def parse(line: String) = {
    try {
      ...
      val Version = if(len > 64) fields(64) else "-1"
      val ProcedureType = if(len > 65) fields(65) else "-1"

      val Token = if(len > 66) fields(6) else "-1"
      val Requst_Action = if(len > 67) fields(67) else "-1"
      val Response_result = if(len > 68) fields(68) else "-1"
      val Status = if(len > 69) fields(69) else "-1"
      val FirstRequestTime = if(len > 70) fields(70) else "-1"

      val AckTime = if(len > 71) fields(71) else "-1"
      val LastRequestTime = if(len > 72) fields(72) else "-1"
      val Retrans_UL_Count = if(len > 73) fields(73) else "-1"
      val Retrans_DL_Count = if(len > 74) fields(74) else "-1"
      val Direction = if(len > 75) fields(75) else "-1"

      val Uri_Port = if(len > 76) fields(76) else "-1"
      val Destination_URI = if(len > 77) fields(77) else "-1"
      val Host = if(len > 78) fields(78) else "-1"
      val Proxy_URI = if(len > 79) fields(79) else "-1"
      val Proxy_Scheme = if(len > 80) fields(80) else "-1"
      val MSGID = if(len > 81) fields(81) else "-1"// new add
      ...
      Row(Interface,IMSI,MSISDN,IMEI,APN,
        ...
        TCPSynNumber,TCPConnetState,SessionStopFlag,Roma_Province,Version,ProcedureType,//new add
